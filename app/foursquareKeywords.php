...
try{
	...
	# Retrieving sales information from chicago 
	foreach(($bank->getSalesList()) as $sale){
		$sale = rtrim($sale);
		$tips = $fs->get('/tips/search',array('ll'=>$lat.','.$lon,'query'=>$sale));
		...
		foreach($json_text->response->tips as $tip){
			...
			$date = date("Y/m/d"); # Need to fix this
			...
			mysql_query("insert into Comments (username,comment,location,date,category,source,gender) 
				values ('$firstname $lastname','$text','$name','$date','Sales','Foursquare','$gender')");
			mysql_query("insert into Images (username,url,date,category) 
				values ('$firstname $lastname','$photo','$date','Sales')");
		}
	}
	mysql_close();	
